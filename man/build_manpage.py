...
class build_manpage(Command):
    ...
    user_options = [("output=", "O", "output file"), ("parser=", None, "module path to optparser (e.g. mymod:func")]
    ...
    def _write_header(self):
        ...
        ret = []
        ret.append(
            '.TH %s 1 %s "%s v.%s"\n' % (self._markup(appname), self._today.strftime("%Y\\-%m\\-%d"), appname, version)
        )
        ...
        if synopsis:
            ...
            synopsis = re.sub(r" +%s" % appname, ".B %s\n.R" % self._markup(appname), synopsis)
    ...
    def _write_footer(self):
        ret = []
        author = "%s <%s>" % (self.distribution.get_author(), self.distribution.get_author_email())
