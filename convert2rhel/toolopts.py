...
class CLI(object):
    ...
    def _get_argparser():
        ...
        return optparse.OptionParser(conflict_handler="resolve", usage=usage, add_help_option=False)

    def _register_options(self):
        """Prescribe what command line options the tool accepts."""
        self._parser.add_option("-h", "--help", action="help", help="Show " " help message and exit.")
        ...
        group.add_option(
            "-v",
            "--variant",
            help="The RHEL variant to which"
        ...
        group = optparse.OptionGroup(
            self._parser, "Automation Options", "The following options are used to" " automate the installation"
        )
        group.add_option(
            "-r",
            "--restart",
        ...
        group.add_option("-y", help="Answer yes to all yes/no questions the" " tool asks.", action="store_true")
        self._parser.add_option_group(group)
    ...
    def _process_cli_options(self):
        ...
        if parsed_opts.variant:
            if not rhelvariant.is_variant_supported(parsed_opts.variant):
                loggerinst.critical(
                    "Error: Incorrect RHEL variant has been "
                    " passed through command line.\nSupported"
                    " variants are: %s." % ", ".join(rhelvariant.get_supported_variants())
                )
...
def print_non_interactive_opts():
    ...
    """
    loggerinst = logging.getLogger(__name__)
    loggerinst.info("For the non-interactive use of the tool, run the" " following command:")
    global tool_opts  # pylint: disable=C0103
