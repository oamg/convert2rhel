...
from convert2rhel.unit_tests import (
    ...
    GetInstalledPkgsWDifferentkey_idMocked,
    RemovePkgsMocked,
    RunSubprocessMocked,
...
class TestPreserveOnlyRHELKernel:
    ...
    def test_preserve_only_rhel_kernel(self, pretend_os, monkeypatch):
        ...
        assert pkghandler.get_installed_pkgs_by_key_id.call_count == 1
...
class TestIsRHELKernelInstalled:
    def test_is_rhel_kernel_installed_no(self, monkeypatch):
        monkeypatch.setattr(pkghandler, "get_installed_pkgs_by_key_id", lambda x, name: [])
...
def test_get_packages_to_remove(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        GetInstalledPkgsWDifferentkey_idMocked(side_effect=different_key_ids_for_packages_to_remove),
    )
...
def test_install_rhel_kernel(subprocess_output, is_only_rhel_kernel, expected, pretend_os, monkeypatch):
    ...
    monkeypatch.setattr(
        pkghandler,
        "get_installed_pkgs_w_different_key_id",
        GetInstalledPkgsWDifferentkey_idMocked(pkg_selection=pkg_selection),
    )
...
def test_install_rhel_kernel_already_installed_regexp(subprocess_output, pretend_os, monkeypatch):
    ...
    monkeypatch.setattr(
        pkghandler,
        "get_installed_pkgs_w_different_key_id",
        GetInstalledPkgsWDifferentkey_idMocked(pkg_selection="kernels"),
    )

    pkghandler.install_rhel_kernel()

    assert pkghandler.get_installed_pkgs_w_different_key_id.call_count == 1
