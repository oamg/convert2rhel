...
def submgr_enable_repos(repos_to_enable):
    ...
    if ret_code != 0:
        logger.critical("Repositories were not possible to enable through subscription-manager:\n%s" % output)
...
def lock_releasever_in_rhel_repositories():
    ...
    if system_info.eus_system and not tool_opts.no_rhsm:
        ...
        if ret_code != 0:
            ...
            logger.info("RHEL repositories locked to the %s minor version." % system_info.releasever)
    else:
        logger.info("Skipping locking RHEL repositories to a specific EUS minor version.")
