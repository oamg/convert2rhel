...
def _get_partition(directory):
    ...
    if ecode or not stdout:
        logger.error(f"grub2-probe returned {ecode}. Output:\n{stdout}")
...
def get_device_number(device):
    ...
    if not output:
        raise BootloaderError(f"The '{device}' device has no PART_ENTRY_NUMBER")
...
def _add_rhel_boot_entry(efibootinfo_orig):
    ...
    logger.debug(f"Block device: {blk_dev!s}")
    logger.debug(f"ESP device number: {dev_number!s}")
    ...
    label = f"Red Hat Enterprise Linux {systeminfo.system_info.version.major!s}"
    ...
    if ecode:
        logger.debug(f"efibootmgr output:\n-----\n{stdout}\n-----")
...
def _remove_orig_boot_entry(efibootinfo_orig, efibootinfo_new):
    ...
    if not efibin_path_orig:
        logger.warning(
            f"Skipping the removal of the original, currenly used bootloader entry '{orig_boot_entry.boot_number}' ({orig_boot_entry.label}):"
            " Unable to get path of its UEFI binary file."
        )
        return
    ...
    if os.path.exists(efibin_path_orig) and efibin_path_orig != efibin_path_new:
        logger.warning(
            f"Skipping the removal of the original, currenly used bootloader entry '{orig_boot_entry.boot_number}' ({orig_boot_entry.label}):"
            f" Its UEFI binary file still exists: {efibin_path_orig}"
        )
        return
...
def _log_critical_error(title):
    logger.critical(
        ...
        "The entry should point to '\\EFI\\redhat\\shimx64.efi'."
    )
